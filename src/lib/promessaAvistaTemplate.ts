...
import { formatDateLower, buildRegimeSuffix } from '@/lib/compromisso-helpers'
...
export function buildPromessaAvistaTemplateData(
  ...
  const blocoIntervenienteAnuente = data.has_interveniente
  ...
  const blocoAssinaturaInterveniente = data.has_interveniente
  ...
  return {
    ...
    vendedor_regime_sufixo: buildRegimeSuffix(
    ...
    comprador_regime_sufixo: buildRegimeSuffix(
    ...
    comissao_responsavel: comissaoResponsavelText,
    prazo_certificado: data.prazo_certificado,
    prazo_reforco: data.prazo_reforco,
    data_escritura: formatDateLower(escrituraDate),
