...
} from '@/lib/promessaFgtsHelpers'
import { buildPromessaFgtsTemplateData } from '@/lib/promessaFgtsTemplate'
...
// useNegocioSync.
const GRUPOS_FGTS = ['vendedores', 'compradores', 'anuentes'] as const
...
export function PromessaFgtsForm() {
  ...
  const { registrarNegocio, calcular, gravar, negocioAtual } = useNegocioSync(form, GRUPOS_FGTS)
  ...
  const valorFgts = useWatch({ control, name: 'valor_fgts' })
  ...
  const saldo = useMemo(() => {
    ...
    const f = parseCurrency(valorFgts || '')
  ...
  const vendedoresW =
    (useWatch({ control, name: 'vendedores' }) as PromessaFgtsValues['vendedores']) || []
  ...
  const onSubmit = async (data: PromessaFgtsValues) => {
    if (!hasBroker) {
  ...
  const onValidate = async () => {
    ...
    try {
      const texto = await getPromessaFgtsText(buildPromessaFgtsTemplateData(form.getValues()))
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-primary">FGTS e Saldo</h3>
          </div>
          <Separator />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
