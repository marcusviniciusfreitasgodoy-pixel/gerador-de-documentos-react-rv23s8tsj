...
// useNegocioSync.
const GRUPOS_FINANCIADA = ['vendedores', 'compradores', 'anuentes'] as const
...
export function PromessaFinanciadaForm() {
  ...
  const { registrarNegocio, calcular, gravar, negocioAtual } = useNegocioSync(
    form,
    GRUPOS_FINANCIADA,
  )
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Button
          ...
          onClick={() => {
            form.reset(promessaFinanciadaMockData)
            aplicarBroker()
          }}
        >
